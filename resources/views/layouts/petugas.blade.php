...
<head>
    ...
    <title>@yield('title') - Petugas SPP</title>
...
<body class="bg-gradient-to-br from-green-50 via-emerald-50 to-teal-100 min-h-screen">
    <div class="min-h-screen flex flex-col">
        ...
        <header class="bg-white/80 shadow-lg sticky top-0 z-30 glass">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between h-16 items-center">
                <div class="flex items-center space-x-4">
                    <button id="sidebarToggle" class="md:hidden text-gray-600 hover:text-emerald-600 focus:outline-none">
                        <i class="fas fa-bars fa-lg"></i>
                    </button>
                    <span class="text-emerald-700 font-extrabold text-2xl tracking-wide drop-shadow">SPP Petugas</span>
                </div>
                <div class="flex items-center space-x-4">
                    <div class="flex items-center space-x-2">
                        <img src="https://i.pravatar.cc/40?img=5" alt="Petugas" class="w-9 h-9 rounded-full border-2 border-emerald-400 shadow">
                        <span class="font-semibold text-gray-700 hidden sm:inline">{{ Auth::user()->name ?? 'Petugas' }}</span>
                    </div>
                    <a href="{{ url('/') }}" class="text-gray-600 hover:text-emerald-600 px-3 py-2 rounded-md text-sm font-medium transition">Beranda</a>
                </div>
            </div>
        </header>
        ...
        <div class="flex-1 flex overflow-hidden">
            <!-- Sidebar Desktop -->
            <aside id="sidebar" class="glass bg-gradient-to-b from-emerald-500/90 to-teal-600/90 text-white w-64 px-2 py-6 hidden md:block transition-all duration-300 h-screen fixed top-16 left-0 z-30 shadow-2xl">
                <nav class="space-y-2">
                    ...
                    <a href="{{ route('petugas.laporan') }}" class="flex items-center px-4 py-2 rounded-xl hover:bg-emerald-700/80 hover:scale-105 transition-all duration-200 {{ request()->routeIs('petugas.laporan') ? 'bg-emerald-700/90' : '' }}">
                        <i class="fas fa-file-alt mr-3 sidebar-icon"></i> <span class="sidebar-label">Laporan</span>
                    </a>
                    <form action="{{ route('logout') }}" method="POST" class="m-0">
                        @csrf
                        <button type="submit" class="w-full flex items-center px-4 py-2 rounded-xl hover:bg-emerald-700/80 hover:scale-105 transition-all duration-200">
            ...
            <div id="sidebarMobileOverlay" class="sidebar-mobile-overlay hidden md:hidden">
                <aside id="sidebarMobile" class="glass bg-gradient-to-b from-emerald-500/90 to-teal-600/90 text-white w-64 px-2 py-6 h-full shadow-2xl sidebar-mobile relative">
                    <button id="closeSidebarMobile" class="absolute top-4 right-4 text-white text-2xl focus:outline-none hover:text-emerald-200">
