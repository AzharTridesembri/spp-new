...
<div class="bg-white rounded-lg shadow-md">
    <div class="p-6 flex flex-col md:flex-row md:justify-between md:items-center border-b gap-4">
        ...
        <div class="flex space-x-2">
            <a href="{{ route('admin.pembayaran.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition flex items-center gap-2">
                <i class="fas fa-plus"></i> <span class="hidden sm:inline">Tambah Pembayaran</span>
            </a>
            <a href="{{ route('admin.laporan') }}" class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition flex items-center gap-2">
                ...
                @forelse ($pembayaran as $index => $item)
                    <tr class="hover:bg-blue-50 transition">
                        ...
                        <td class="py-3 px-4 flex items-center justify-center space-x-2">
                            <a href="{{ route('admin.pembayaran.show', $item->id) }}" class="p-2 bg-blue-100 text-blue-600 rounded hover:bg-blue-200" title="Detail">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="{{ route('admin.pembayaran.edit', $item->id) }}" class="p-2 bg-yellow-100 text-yellow-700 rounded hover:bg-yellow-200" title="Edit">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('admin.pembayaran.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')" class="inline">
                                @csrf
                                @method('DELETE')
