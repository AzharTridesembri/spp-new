...
<div class="bg-white rounded-lg shadow-md">
    <div class="p-6 flex flex-col md:flex-row md:justify-between md:items-center border-b gap-4">
        ...
        <a href="{{ route('admin.users.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg flex items-center gap-2">
        ...
        <table class="min-w-full bg-white rounded-lg overflow-hidden text-sm">
            ...
            <tbody class="text-gray-600 divide-y">
                ...
                <tr class="hover:bg-blue-50 transition">
                    ...
                    <td class="py-4 px-4 font-semibold text-blue-700">{{ $item->name }}</td>
                    ...
                    <td class="py-4 px-4 flex items-center justify-center space-x-2">
                        ...
                        <form action="{{ route('admin.users.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus user ini?')">
                            @csrf
                            @method('DELETE')
